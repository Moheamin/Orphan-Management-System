          ...
          <tbody>
            {payments.map((p) => (
              ...
              >
                ...
                <td
                  className={`px-4 py-2 text-xs font-mono ${p.remaining_debt > 0 ? "text-[var(--errorColor)]" : "text-[var(--successColor)]"}`}
                >
                  {p.remaining_debt > 0 ? "-" : ""}
...
function OrphanRow({ item }: { item: any }) {
  ...
  const s = STATUS_CFG[item.funding_status] ?? STATUS_CFG["Orphanage Only"];
  ...
  return (
    <>
      <DataTable.TableRow>
        ...
        {/* Received */}
        <DataTable.TableCell className="hidden lg:table-cell font-mono text-sm font-bold text-[var(--successColor)] text-right">
        ...
        <DataTable.TableCell>
          <button
            onClick={() => setOpen((v) => !v)}
            className="p-1 rounded-md hover:bg-[var(--fillColor)] text-[var(--textMuted)] transition-colors"
            title="عرض الكفلاء"
          >
...
function OrphanReceivesContent() {
  ...
  const totals = useMemo(
    () => ({
      target: filtered.reduce((s, o) => s + o.total_monthly_target, 0),
      received: filtered.reduce((s, o) => s + o.total_actual_received, 0),
      gap: filtered.reduce((s, o) => s + o.financial_gap, 0),
    }),
    [filtered],
  );
...
export default function OrphanReceives() {
  return (
    <div className="mx-4 md:mx-8 mb-8">
      <div className="mt-6 md:mt-8 mr-2 mb-6 md:mb-8 flex flex-col items-end">
        ...
        <p className="mb-4 text-[var(--subTextColor)] text-sm">
          عرض تفاصيل استقبالات الأيتام من الكفلاء والدار
        </p>
      </div>
      <OrphanReceivesTable />
    </div>
